...
#===========================================
  def fitness(self,x):
    ...
    with open('{}/output_gen{}_{}.log'.format(options.root,self.generation+1,self.id(x)),'a') as file:
      ...
      orientations = np.array([ [ 301.5, 82.9, 119.0 ],
                              ])
      error = []
  ...
  #--------- Running damask postProcessing and finding error against "experimental" data ----------- #
        ...
        if not os.path.isfile('{}/{}.sta'.format(dir_loc,self.job_id)):
          ...
          if exit_status != "SUCCESSFULLY":
            ...
            cmd7 = 'abaqus_InterpolatedImage.py -c coordinate --rescale 1.0 --incomplete --stop_depth 50.0 --final_depth 361.0 --radius 1.0 --rotation 90.0 --grid 256 256 -p 10.0/255 {}/coord_after_retracting.txt'.format(dir_loc)
            run([cmd7],shell=True)
            print(cmd7)
