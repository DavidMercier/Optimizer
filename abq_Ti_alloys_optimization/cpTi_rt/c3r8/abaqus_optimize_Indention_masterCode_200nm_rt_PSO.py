...
#===========================================
  def fitness(self,x):
    ...
    with open('{}/output_gen{}_{}.log'.format(options.root,self.generation+1,self.id(x)),'a') as file:
      ...
      orientations = np.array([ [ 124.420, 82.564, 255.02 ],
                          ])                            
      error = []
  ...
  #--------- Running damask postProcessing and finding error against "experimental" data ----------- #
        ...
        if not os.path.isfile('{}/{}.sta'.format(dir_loc,self.job_id)):
          ...
          if exit_status != "SUCCESSFULLY":
            ...
            cmd7 = 'abaqus_InterpolatedImage.py -c coordinate --rescale 1.0 --incomplete --stop_depth 200.0 --final_depth 540.0 --radius 1.0 --rotation 90.0 --grid 256 256 -p 10.0/255 {}/coord_after_retracting.txt'.format(dir_loc)
            run([cmd7],shell=True)
            print(cmd7)

            cmd8 = 'subset_image.py -d 256 256 --rmin 0 --rmax 70 -l 3_gridInterpolation --positive {}/out_coord_after_retracting.txt'\
                        .format(dir_loc)
            run([cmd8], shell=True)
            print(cmd8)
...
if options.restart:
  ...
  theOptimizer = optimize( method = 'pso',
                           ...
                           localbest_rs      = np.array([0.67441577,0.38738156,0.23923753,0.25563915,0.32511285, \
                                                         0.29750664,0.26635067,0.40625859,0.28562731,0.47781169, \
                                                         0.2617993,0.32868611,0.27858981,0.30812319,0.47366377, \
                                                         0.46384936,0.36350501,0.23718267,np.inf,np.inf]),        # array of history best fitnesses of each particle for current generation
                           gbest_rs          = 0.2371826735323323,                                                        # history global best fitness for current generation 
                           )
else:
  theOptimizer = optimize( method = 'pso',
                           bounds = np.array([[10e6,500e6],                     
