...
#===========================================
  def fitness(self,x):
    ...
    with open('{}/output_gen{}_{}.log'.format(options.root,self.generation+1,self.id(x)),'a') as file:
      ...
      orientations = np.array([ [ 188.817, 89.173, 185.43 ],
                          ])                            
      error = []
  ...
  #--------- Running damask postProcessing and finding error against "experimental" data ----------- #
        ...
        if not os.path.isfile('{}/{}.sta'.format(dir_loc,self.job_id)):
          ...
          if exit_status != "SUCCESSFULLY":
            ...
            cmd5 = 'chopTable.py -l "depth/nm" -v 200.0 {}/abaqus_result_load_disp.txt'.format(dir_loc)
            run([cmd5],shell=True)
            print(cmd5)
        
            cmd6 = 'InterpolateTables.py --expX "exp_depth/nm" --expY "exp_load/mN" --expfile {}/exp_load_disp_250c_{}.txt --simX "depth/nm" --simY "load/mN" --simfile {}/chopped_abaqus_result_load_disp.txt'\
                   .format(self.root,orientation_run+1,dir_loc)
            load_disp_error = check_output(cmd6, shell=True, universal_newlines=True)
            print(cmd6)
        
            cmd7 = 'abaqus_InterpolatedImage.py -c coordinate --rescale 1.0 --incomplete --stop_depth 200.0 --final_depth 522.0 --radius 1.0 --rotation 90.0 --grid 256 256 -p 8.0/255 {}/coord_after_retracting.txt'.format(dir_loc)
            run([cmd7],shell=True)
            print(cmd7)

            cmd8 = 'subset_image.py -d 256 256 --rmin 10 --rmax 70 -l 3_gridInterpolation --positive {}/out_coord_after_retracting.txt'\
                        .format(dir_loc)
            run([cmd8], shell=True)
            print(cmd8)

            cmd9 = 'abaqus_compare_pileup.py --error --multiplier 1e6 --flipud --sim {}/subset_sim.txt {}/exp_pileUp_250c_{}.txt '\
...
if options.restart:
  ...
  theOptimizer = optimize( method = 'pso',
                           ...
                           localbest_rs      = np.array([0.2334961,0.23298106,0.26685659,0.24004752,0.23037869, \
                                                         0.22762214,0.2284383,0.22845072,0.22810388,0.23268532, \
                                                         0.22860171,0.27406265,0.29112631,0.23023769,0.27384437, \
                                                         0.23691372,0.22802465,0.23093911,0.22779686,0.26821807]),        # array of history best fitnesses in each particle
                           gbest_rs          = 0.2276221408066548,                                                        # history global best fitness 
                           )
else:
  theOptimizer = optimize( method = 'pso',
                           bounds = np.array([[10e6,500e6],                     
