...
#===========================================
  def fitness(self,x):
    ...
    with open('{}/output_gen{}_{}.log'.format(options.root,self.generation+1,self.id(x)),'a') as file:
      ...
      orientations = np.array([ [ 47.433, 78.097, 292.32 ],
                          ])                            
      error = []
  ...
  #--------- Running damask postProcessing and finding error against "experimental" data ----------- #
        ...
        if not os.path.isfile('{}/{}.sta'.format(dir_loc,self.job_id)):
          ...
          if exit_status != "SUCCESSFULLY":
            ...
            self.curr_locations.append(np.append(x,np.nan))
            self.locations[self.id(x)] = np.nan
            return np.nan
            ...
            cmd7 = 'abaqus_InterpolatedImage.py -c coordinate --rescale 1.0 --incomplete --stop_depth 200.0 --final_depth 518.0 --radius 1.0 --rotation 90.0 --grid 256 256 -p 8.0/255 {}/coord_after_retracting.txt'.format(dir_loc)
            run([cmd7],shell=True)
            print(cmd7)
            ...
            cmd9 = 'abaqus_compare_pileup.py --multiplier 1e6 --flipud --sim {}/subset_sim.txt {}/exp_pileUp_250c_{}.txt '\
...
#------------------------------------------------------------------------------------------------- #
      avg_error = np.mean(error)                                                                         # 2 for each orientation
...
if options.restart:
  ...
  theOptimizer = optimize( method = 'pso',
                           ...
                           localbest_rs      = np.array([0.51447839,0.58132769,0.53090445,0.53328172,0.51516947,\
                                                         ...
                                                         0.53126051,0.51738898,0.60547042,0.5555745,0.55399484]),        # array of history best fitnesses in each particle
                           gbest_rs          = 0.5144783867015663,                                                        # history global best fitness 
                           )
else:
  theOptimizer = optimize( method = 'pso',
                           bounds = np.array([[10e6,500e6],                     
