...
#===========================================
  def fitness(self,x):
    ...
    with open('{}/output_gen{}_{}.log'.format(options.root,self.generation+1,self.id(x)),'a') as file:
      ...
      orientations = np.array([ [ 91.149, 79.295, 268.80 ],
                              ])
      error = []
...
#           shutil.copy("%s/%s"%(self.root,self.exp_file),'%s/'%dir_loc)
  ...
  #--------- Running damask postProcessing and finding error against "experimental" data ----------- #
        ...
        for i in [2,3,5,6,7,8,9]:
          ...
          cores = run(cmd0, capture_output=True, shell=True, text=True).stdout
          cmd0_1 = 'ssh compute{:02}.egr.msu.edu -t "nc --recv-only localhost 51001"'.format(i)
          load = run(cmd0_1, capture_output=True, shell=True, text=True).stdout
        ...
        if not os.path.isfile('{}/{}.sta'.format(dir_loc,self.job_id)):
          ...
          if exit_status != "SUCCESSFULLY":
            ...
            load_disp_error = run(cmd6, capture_output=True, shell=True, text=True).stdout
            print(cmd6)
        
            cmd7 = 'abaqus_InterpolatedImage.py -c coordinate --rescale 1.0 --incomplete --stop_depth 200.0 --final_depth 550.0 --radius 1.0 --rotation 90.0 --grid 256 256 -p 10.0/255 {}/coord_after_retracting.txt'.format(dir_loc)
            run([cmd7],shell=True)
            print(cmd7)
            ...
            pile_up_error = run(cmd9, capture_output=True, shell=True, text=True).stdout
            print(cmd9)
