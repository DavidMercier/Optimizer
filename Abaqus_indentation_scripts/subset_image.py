...
scriptID   = str.replace('$Id: subset_image.py 154 2015-11-06 14:33:26Z chakra34 $','\n','\\n')
...
parser.add_option('-c','--center',
                  ...
                  help = 'center of circular cropping [image center] (y,x)')
...
for name in filenames:
  ...
  if options.center == None:
    options.center = [int(options.dimension[0]/2),int(options.dimension[1]/2)]
  ...
  for j in range(options.dimension[0]):
    for i in range(options.dimension[1]):
  ...
  subset_outputName = os.path.join(os.path.dirname(name),'subset_sim.txt')
  with open(subset_outputName,'w') as subset_output:
    subset_output.write("1 head\n")
    subset_output.write("values\n")
  temp_data = np.ones((options.new_d[0],options.new_d[1]))
  for i in range(-options.new_d[0],options.new_d[0]):
    for j in range(-options.new_d[1],options.new_d[1]):
      temp_data[i,j] = data[options.center[0] + i, options.center[1]+ j]
      with open(subset_outputName,'a') as subset_output:
